...
(() => {
  document.addEventListener('DOMContentLoaded', () => {
    // Run all processes
    Object.values(processes).forEach(p => p());
  });
      ...
      for (const blockquote of blockquotes) {
        ...
        for (const alert of alerts) {
          if (text.toLowerCase().includes(`[!${alert}]`)) {
            blockquote.classList.add('markdown-alert', `markdown-alert-${alert}`);
    
            const contents = blockquote.innerHTML.replace(/\[!.+?\]/i, '');
            blockquote.innerHTML = `
              <p class="markdown-alert-title">
                <svg class="octicon mr-2" class="markdown-alert-icon" viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true">
                  <path d="${icons[alert]}"></path>
                </svg>
